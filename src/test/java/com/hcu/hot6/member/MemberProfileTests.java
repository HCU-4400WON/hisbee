...
public class MemberProfileTests {
    ...
    @PostConstruct
    void setup() {
        Member member = Member.builder().uid("1").email(TEST_EMAIL).pictureUrl("").build();
    ...
    public void 유저_정보가_표시_되어야_한다() throws Exception {
        // given
        MemberRequest req =
                MemberRequest.builder().nickname("monsters").major1(Major.GLS).major2(Major.NONE).build();
        ...
        MvcResult mvcResult =
                mvc.perform(
                                post("/users")
                                        ...
                                        .content(form)
                                        .with(oauth2Login().attributes(attr -> attr.put("sub", TEST_EMAIL))))
    ...
    public void 내가_쓴_글을_불러올_수_있다() throws Exception {
        // given
        MemberRequest form = MemberRequest.builder().nickname("monsters").major1(Major.GLS).build();
        memberService.updateMember(TEST_EMAIL, form);

        var post = PostCreationRequest.builder().title("lorem ipsum").build();
    ...
    public void 좋아요한_글을_불러올_수_있다() throws Exception {
        ...
        var req = PostCreationRequest.builder().title("lorem ipsum").build();
    ...
    public void 유저_정보는_수정될_수_있다() throws Exception {
        ...
        // when
        MemberRequest req2 =
                MemberRequest.builder().nickname("godisgood").major1(Major.CS).major2(Major.EE).build();
