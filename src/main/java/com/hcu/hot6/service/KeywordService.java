    ...
    public List<Keyword> addKeywords(Post post, PostCreationRequest request) {
        ...
        final List<String> labels =
                new ArrayList<>(request.getKeywords())
                        .stream().filter(label -> !post.getPostTypes().contains(label)).toList();
        ...
        keywords.forEach(keyword -> postKeywordRepository.save(new PostKeyword(post, keyword)));
        return keywords;
    }
