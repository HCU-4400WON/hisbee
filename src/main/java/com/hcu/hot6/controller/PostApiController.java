...
public class PostApiController {
    ...
    public ResponseEntity<PostReadOneResponse> readOnePost(
            @PathVariable Long postId, @AuthenticationPrincipal OAuth2User user) {
    ...
    /** 모집글 필터링 전체 조회 */
    @GetMapping("/posts")
    public ResponseEntity<PostFilterResponse> readFilteredPost(
        ...
        return ResponseEntity.ok(
                new PostFilterResponse(
                        posts.size(),
                        suggestions,
                        posts.stream().map(post -> post.getThumbnail().toResponse(email)).toList()));
    }

    /** 아카이브된 모집글 조회 */
    @GetMapping("/posts/archived")
    public ResponseEntity<PostFilterResponse> readAchivedPosts(
            @AuthenticationPrincipal OAuth2User user) {
        String email = (Objects.isNull(user)) ? "" : user.getName();
