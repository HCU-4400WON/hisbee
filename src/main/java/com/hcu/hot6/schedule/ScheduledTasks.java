...
public class ScheduledTasks {
    ...
    public void midnightPostClose() {
        ...
        PostSearchFilter filter =
                PostSearchFilter.builder().isClosed(false).orderBy(OrderBy.END).build();

        postRepository.findAll(filter, null).stream()
                .filter(post -> post.getThumbnail().getRecruitEnd().toLocalDate().isBefore(now))
