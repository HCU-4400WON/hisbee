import { IPostExample } from "./PostExamples";




function Thumbnail({duration , title, subTitle , categories , keywordsFirstLine , keywordsSecondLine , Likes} : IPostExample){
    ...
    return(<div className="min-w-[310px] h-[260px] p-[15px] bg-white rounded-xl border mx-[20px]">
        ...
        <div className="flex mt-[25px]">
            {categories.map((category, index) => (
                <span key={index} className="px-[10px] py-[1px] bg-blue-100 mr-[10px] rounded-full text-[15px]">{category}</span>
            ))}
        </div>

        <div className="flex mt-[10px] w-full">
        {keywordsFirstLine.map((keyword,index) => (
            <span key={index} className="px-[10px] py-[1px] bg-blue-200 mr-[10px] rounded-full text-[15px]">{keyword}</span>
        ))}
        </div>
